 ...
 * @since  [产品/模块版本]
 */
@Path("ec/eserver/addGroup")
public class AddGroupResource
{
    /**
     * 日志对象
     */
    private static final Logger LOGGER = Logger.getLogger(AddGroupResource.class);
    ...
    public RestResponse<String> addGroup(GroupRequest request)
    {
        RestResponse<String> response = new RestResponse<String>();
        
        // 参数判空
        if (null == request || StringUtils.isEmpty(request.getSenderAppID()) || StringUtils.isEmpty(request.getName())
            || StringUtils.isEmpty(request.getOwner()))
        {
        ...
        try
        {
            SDKResult<String> result = GROUP_SERVICE.addGroup(GROUP_RESOURCE_CONVERT.buildGroup(request));
            // 转换结果
        ...
        {
            LOGGER.error("addGroup method SDK error", e);
        ...
        {
            LOGGER.error("addGroup method error", e);
