 ...
 * @since  [产品/模块版本]
 */
@Path("ec/eserver/getIMGroup")
public class GetIMGroupResource
{
    /**
     * 日志对象
     */
    private static final Logger LOGGER = Logger.getLogger(GetIMGroupResource.class);
    ...
    public RestResponse<String> getIMGroup(@QueryParam("queryKey") String queryKey,
        @QueryParam("isNeedAmount") String isNeedAmount, @QueryParam("count") String count,
        @QueryParam("senderAppID") String senderAppID, @QueryParam("senderType") String senderType,
        @QueryParam("pageIndex") String pageIndex)
    {
        RestResponse<String> response = new RestResponse<String>();
        
        // 参数判空
        if (StringUtils.isEmpty(senderAppID))
        {
        ...
        try
        {
            SDKResult<String> result =
                GROUP_SERVICE.getIMGroup(queryKey, isNeedAmount, count, senderAppID, senderType, pageIndex);
        ...
        {
            LOGGER.error("getIMGroup method SDK error", e);
        ...
        {
            LOGGER.error("getIMGroup method error", e);
