...
@Path("ec/eserver/appSendMsgToUC")
public class AppSendMsgToUCResource
{
    ...
    public RestErrCode appSendMsgToUC(AppSendMsgToUC msg)
    {
         ...
         */
        if (null == msg || StringUtils.isEmpty(msg.getUcAccount()) || StringUtils.isEmpty(msg.getSendNumber())
            || null == msg.getDateTime() || StringUtils.isEmpty(msg.getMessage()))
        {
        ...
        try
        {
            UCUserInstanceMessage im = convert.appSendMsgToUCRest2Modal(msg);
            SDKErrorCode response = imService.appSendMsgToUC(im);
        ...
        {
            LOGGER.error("appSendMsgToUC method SDK error", e);
            result.setResultCode(String.valueOf(e.getSdkErrCode()));
            return result;
        }
        catch (Exception e)
        {
            LOGGER.error("appSendMsgToUC method error", e);
