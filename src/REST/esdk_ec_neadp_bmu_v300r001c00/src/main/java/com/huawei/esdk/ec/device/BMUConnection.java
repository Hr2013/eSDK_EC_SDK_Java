...
import com.huawei.esdk.ec.device.bmu.bean.UserAuthResBean;
...
public class BMUConnection extends UCDeviceConnectionBase
{
    private static final Logger LOGGER = Logger.getLogger(BMUConnection.class);
    
    private MultiConnDeviceBase device;
    
    private ISDKProtocolAdapter protocolAdapter;
    
    public BMUConnection(ISDKProtocolAdapter protocolAdapter, MultiConnDeviceBase serviceProxy, String user, String pwd)
    {
        super(user, pwd);
    ...
    public boolean initConn(String connId)
    {
        ...
        {
            XMLResMsg result =
                (XMLResMsg)protocolAdapter.syncSendMessage(req, "/appAuth.action", UserAuthResBean.class.getName());
