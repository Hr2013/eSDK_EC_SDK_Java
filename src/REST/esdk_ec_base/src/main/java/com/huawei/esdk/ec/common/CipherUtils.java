...
public class CipherUtils
{
        ...
        throws SDKException
    {
        if (ESDKConstant.SENSITIVE_INFO_TRANSMISSION_MODE_AES128_CONSULTED.equalsIgnoreCase(model))
        {
            ...
            if (null != mc)
            {
                String ipAppId = (String)mc.getEntities().get(ESDKConstant.ESDK_CLIENT_IP);
                AccountInfo acctInfo = (AccountInfo) mc.getEntities().get(ESDKConstant.ACCT_INFO_ESDK);
                ipAppId = ipAppId + "_" + acctInfo.getUserId();
                
                com.huawei.esdk.ec.authorize.SessionInfo sessionInfo = sessionMgr.getSDKSession(ipAppId);
                if (null == sessionInfo || 0 == sessionInfo.getSecretKey().length || 0 == sessionInfo.getIv().length)
                {
                ...
                aes128.setBT_KEY(sessionInfo.getSecretKey());
                aes128.setBT_IV(sessionInfo.getIv());
                try
                {
