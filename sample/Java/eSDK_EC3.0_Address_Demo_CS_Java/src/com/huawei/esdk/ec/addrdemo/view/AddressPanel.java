...
public class AddressPanel extends JPanel {
	...
	private JLabel accountLabel = new JLabel("操作者EC帐号：");
	private JLabel conditionLabel = new JLabel("查询条件：");
	private JLabel stafaccountLabel = new JLabel("被查询EC帐号：");
	...
	private JTextField stafaccountfield = new JTextField(10);
	...
	private JButton getaddr = new MyButton("查询个人通讯录");
	//private JButton getperinfo = new MyButton("查询个人详情");
	private JButton getent = new MyButton("查询企业通讯录");
	//private JButton getemp = new MyButton("查询个人信息");
	...
	private JPanel getPanels() {
		...
		buildPanel(panel, gridbag, c, new JComponent[] { accountLabel }, 0, 3, 10, 20, 1, 1);
		buildPanel(panel, gridbag, c, new JComponent[] { accountfield }, 1, 3, 10, 20, 1, 1);
		buildPanel(panel, gridbag, c, new JComponent[] { conditionLabel }, 2, 3, 10, 20, 1, 1);
		buildPanel(panel, gridbag, c, new JComponent[] { conditionfield }, 3, 3, 10, 20, 1, 1);
		buildPanel(panel, gridbag, c, new JComponent[] { stafaccountLabel }, 0, 4, 10, 20, 1, 1);
		buildPanel(panel, gridbag, c, new JComponent[] { stafaccountfield }, 1, 4, 10, 20, 1, 1);
		...
		buildPanel(panel, gridbag, c, new JComponent[] { pagecLabel }, 0, 5, 10, 20, 1, 1);
		buildPanel(panel, gridbag, c, new JComponent[] { pagecfield }, 1, 5, 10, 20, 1, 1);
		buildPanel(panel, gridbag, c, new JComponent[] { pagenLabel }, 2, 5, 10, 20, 1, 1);
		buildPanel(panel, gridbag, c, new JComponent[] { pagenfield }, 3, 5, 10, 20, 1, 1);

		buildPanel(panel, gridbag, c, new JComponent[] { getaddr }, 0, 6, 100, 30, 2, 1);
		buildPanel(panel, gridbag, c, new JComponent[] { getent }, 2, 6, 100, 30, 2, 1);
		 ...
		 * createLineBorder(new Color(220, 220, 220), 15), "通讯录管理",
	...
	private void init() {
		...
		// 查询个人通讯录，发起GET请求
		this.getaddr.addMouseListener(new MouseAdapter() {
			public void mouseClicked(MouseEvent e) {
				showErrInfoWithColor("");
				retDescArea.setText("");

				boolean flag = validateParams(
						new JTextField[] { urlField, userfield, pwdField, accountfield, conditionfield, pagecfield,
								pagenfield },
						new String[] { "eSDK URL", "用户名", "密码", "操作者EC账号", "查询条件", "每页显示数", "当前分页" });
		...
		this.getent.addMouseListener(new MouseAdapter() {
			public void mouseClicked(MouseEvent e) {
				showErrInfoWithColor("");
				retDescArea.setText("");

				boolean flag = validateParams(
						new JTextField[] { urlField, userfield, pwdField, DeptIdfield, pagecfield, pagenfield },
						new String[] { "eSDK URL", "用户名", "密码", "部门ID", "每页显示数", "当前分页" });
	...
	private void serchAddr() {
		...
		try {
			...
			String responsePayload = RestUtils.getInstance().sendMessage(restRequestMessage, "/ec/appserver/address");
			finish();
